          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 28 }}>
              <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, padding: "18px 20px" }}>
                <div style={{ fontSize: 11, color: C.textD, marginBottom: 6 }}>Resumes</div>
                <div style={{ fontSize: 32, fontWeight: 700, color: C.accent }}>{resumeVersions.filter(function(v) { return !v.isRefinement; }).length}</div>
              </div>
              <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, padding: "18px 20px" }}>
                <div style={{ fontSize: 11, color: C.textD, marginBottom: 6 }}>Applications</div>
                <div style={{ fontSize: 32, fontWeight: 700, color: C.emerald }}>{apps.length}</div>
              </div>
              <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, padding: "18px 20px" }}>
                <div style={{ fontSize: 11, color: C.textD, marginBottom: 6 }}>Interviews</div>
                <div style={{ fontSize: 32, fontWeight: 700, color: C.amber }}>{apps.filter(function(a) { return a.stage === "interview"; }).length}</div>
              </div>
              <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, padding: "18px 20px" }}>
                <div style={{ fontSize: 11, color: C.textD, marginBottom: 6 }}>Avg match</div>
                <div style={{ fontSize: 32, fontWeight: 700, color: C.purple }}>{resumeVersions.length > 0 ? Math.round(resumeVersions.reduce(function(s, v) { return s + (v.score || 0); }, 0) / resumeVersions.length) + "%" : "\u2014"}</div>
              </div>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 28 }}>
              <button onClick={startNewResume} style={{ padding: "22px 24px", borderRadius: 14, border: "1px solid rgba(79,142,247,0.2)", background: "rgba(79,142,247,0.04)", color: C.text, fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
                <div style={{ fontSize: 18, marginBottom: 8, color: C.accent }}>+</div>
                Tailor new resume
                <div style={{ fontSize: 12, fontWeight: 400, marginTop: 6, color: C.textD }}>Paste a job posting and generate an ATS-optimized resume</div>
              </button>
              <div style={{ padding: "22px 24px", borderRadius: 14, border: "1px solid " + C.border, background: C.surface, color: C.text, fontSize: 15, fontWeight: 600, textAlign: "left" }}>
                <div style={{ fontSize: 18, marginBottom: 8, color: C.emerald }}>{">"}</div>
                Application pipeline
                <div style={{ fontSize: 12, fontWeight: 400, marginTop: 6, color: C.textD }}>
                  {apps.length === 0 ? "Start tracking applications to see your pipeline" : apps.length + " applications tracked"}
                </div>
              </div>
            </div>
            ...
            {apps.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 14 }}>Applications</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                  {["applied", "interview", "offer", "rejected"].map(function(stage) {
                    var stageColor = stage === "applied" ? C.accent : stage === "interview" ? C.amber : stage === "offer" ? C.emerald : C.textD;
                    var stageApps = apps.filter(function(a) { return a.stage === stage; });
                    return (
                      <div key={stage} style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 12, padding: 12, minHeight: 120 }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: stageColor, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>
                          {stage} ({stageApps.length})
                        </div>
                        {stageApps.map(function(app) {
                          return (
                            <div key={app.id} style={{ background: C.bg, border: "1px solid " + C.border, borderRadius: 10, padding: "10px 12px", marginBottom: 8, fontSize: 12 }}>
                              <div style={{ fontWeight: 600, marginBottom: 3 }}>{app.title}</div>
                              <div style={{ fontSize: 11, color: C.textD }}>{app.company}</div>
                              <div style={{ display: "flex", gap: 6, marginTop: 6, alignItems: "center" }}>
                                <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 6, fontWeight: 600, background: app.score >= 80 ? "rgba(45,212,160,0.1)" : "rgba(245,183,49,0.1)", color: app.score >= 80 ? C.emerald : C.amber }}>{app.score}%</span>
                                <select value={app.stage} onChange={function(e) { updateAppStage(app.id, e.target.value); }} style={{ fontSize: 10, background: C.bg, border: "1px solid " + C.border, borderRadius: 6, color: C.textM, padding: "2px 6px", fontFamily: "inherit" }}>
                                  <option value="applied">applied</option>
                                  <option value="interview">interview</option>
                                  <option value="offer">offer</option>
                                  <option value="rejected">rejected</option>
                                </select>
                                <button onClick={function() { deleteApp(app.id); }} style={{ fontSize: 10, background: "none", border: "none", color: C.textD, cursor: "pointer", marginLeft: "auto" }}>x</button>
                              </div>
                            </div>
                          );
                        })}
            ...
            {resumeVersions.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 14 }}>Resume versions</div>
                <div style={{ background: C.surface, border: "1px solid " + C.border, borderRadius: 14, overflow: "hidden" }}>
                  {resumeVersions.slice(0, 10).map(function(ver, i) {
                    return (
                      <div key={ver.id} style={{ padding: "14px 18px", borderBottom: i < Math.min(resumeVersions.length, 10) - 1 ? "1px solid " + C.border : "none", display: "flex", alignItems: "center", gap: 14 }}>
                        <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: ver.score >= 80 ? "rgba(45,212,160,0.08)" : "rgba(245,183,49,0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: ver.score >= 80 ? C.emerald : C.amber }}>
                          {ver.score > 0 ? ver.score + "%" : "\u2014"}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 600 }}>
                            {ver.title}
                            {ver.isRefinement && <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 5, background: C.purpleS, color: C.purple, fontWeight: 600, marginLeft: 8 }}>REFINED</span>}
                          </div>
                          <div style={{ fontSize: 11, color: C.textD, marginTop: 2 }}>
                            {ver.company ? ver.company + " \u2022 " : ""}{new Date(ver.date).toLocaleDateString("en-CA", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                          </div>
                        </div>
                      </div>
                    );
            ...
            {resumeVersions.length === 0 && apps.length === 0 && (
              <div style={{ textAlign: "center", padding: "60px 20px" }}>
                ...
                <div style={{ fontSize: 13, maxWidth: 420, margin: "0 auto", lineHeight: 1.7, color: C.textD }}>
                  Click + New to paste a job posting and generate your first tailored resume.
                </div>
                <button onClick={startNewResume} style={{ marginTop: 20, padding: "10px 28px", borderRadius: 10, border: "none", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", background: C.accent, color: "#fff" }}>Get started</button>
              </div>
            )}
          </div>
        )}
